...
<head>
    ...
    <title>Struk Barang Masuk — {{ $stockIn->transaction_number }}</title>
    <style>
...
<body>
    <div class="receipt-wrapper">
        ...
        <div class="action-bar">
            <a href="{{ route('stock-in.index') }}" class="btn-back">
                ← Kembali
            </a>
        ...
        <div class="receipt">
            ...
            <div class="receipt-header">
                <div class="header-top">
                    ...
                    <div class="badge-type">📦 Barang Masuk</div>
                </div>
                <div class="trx-number">{{ $stockIn->transaction_number }}</div>
            </div>
            ...
            <div class="receipt-body">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <span>{{ $stockIn->date->format('d F Y') }}</span>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <span>{{ $stockIn->product->category->name ?? '—' }}</span>
                    </div>
                </div>
                ...
                <div class="product-box">
                    <div class="product-name">{{ $stockIn->product->name }}</div>
                    <div class="product-code">Kode: {{ $stockIn->product->code }}</div>
                    <div class="product-meta">
                        <div class="meta-item">
                            <span class="meta-label">Satuan</span>
                            <span class="meta-value">{{ $stockIn->product->unit }}</span>
                        </div>
                        <div class="meta-item">
                            <span class="meta-label">Lokasi Rak</span>
                            <span class="meta-value">{{ $stockIn->product->rack_location ?: '—' }}</span>
                        </div>
                        <div class="meta-item">
                            <span class="meta-label">Stok Saat Ini</span>
                            <span class="meta-value">{{ $stockIn->product->current_stock }} {{ $stockIn->product->unit }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="qty-highlight">
                    <span class="qty-label">Jumlah Barang Masuk</span>
                    <div>
                        <span class="qty-value">+{{ $stockIn->quantity }}</span>
                        <span class="qty-unit">{{ $stockIn->product->unit }}</span>
                    </div>
                </div>
                ...
                <div class="notes-box">
                    <p>{{ $stockIn->description }}</p>
                </div>
                @endif

                @if($stockIn->receipt_path)
                @php $ext = strtolower(pathinfo($stockIn->receipt_path, PATHINFO_EXTENSION)); @endphp
                <div class="section-title">Bukti Kwitansi</div>
                <div class="receipt-image-section">
                    @if(in_array($ext, ['jpg','jpeg','png']))
                        <img src="{{ asset('storage/' . $stockIn->receipt_path) }}" alt="Bukti Kwitansi">
                    @else
                        <a href="{{ asset('storage/' . $stockIn->receipt_path) }}" target="_blank" class="receipt-pdf-link">
            ...
            <div class="watermark-strip">
                ✅ Dokumen Resmi — Barang Masuk Gudang
            </div>
        </div>
